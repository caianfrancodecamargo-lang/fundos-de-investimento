...
import base64
import itertools # Para gerar cores dinâmicas
...
# CSS customizado com espaçamentos reduzidos na sidebar e fonte Montserrat
st.markdown("""
<style>
    ...
    /* Sidebar com padding reduzido e verde mais translúcido */
    [data-testid="stSidebar"] {
        background: linear-gradient(180deg, rgba(90, 138, 111, 0.95) 0%, rgba(74, 122, 95, 0.95) 100%); /* Mais translúcido */
        padding: 1rem 0.8rem !important;
        box-shadow: 2px 0 10px rgba(0,0,0,0.1); /* Adicionado sombra para destacar */
    }
    ...
    [data-testid="stSidebar"] .stMultiSelect label, /* NOVO: para o multiselect */
    [data-testid="stSidebar"] .stCheckbox label { /* Ajuste para checkbox */
        color: #ffffff !important;
        font-weight: 600;
    ...
    [data-testid="stSidebar"] .stMultiSelect { /* NOVO: para o multiselect */
    ...
    [data-testid="stSidebar"] [data-testid="stMultiSelect"] div[data-baseweb="select"] { /* NOVO: para o multiselect */
    ...
    [data-testid="stSidebar"] [data-testid="stMultiSelect"] div[data-baseweb="select"] div[data-baseweb="placeholder"] { /* NOVO: para o multiselect */
    ...
    [data-testid="stSidebar"] [data-testid="stMultiSelect"] div[data-baseweb="select"]:focus-within { /* NOVO: para o multiselect */
        color: #000000 !important;
    ...
    [data-testid="stSidebar"] [data-testid="stMultiSelect"] div[data-baseweb="select"]:hover { /* NOVO: para o multiselect */
    ...
    div[data-testid="metric-container"] {
        ...
        border-left: 4px solid var(--secondary-color); /* Usando variável */
    ...
    .stTabs [aria-selected="true"] {
        background: linear-gradient(135deg, var(--secondary-color) 0%, #8ba888 100%); /* Usando variável */
        color: white !important;
        box-shadow: 0 3px 10px rgba(107, 155, 127, 0.3);
    }

    /* Subtítulos */
    h2, h3, h4 { /* Incluído h4 para consistência */
        color: var(--primary-color); /* Usando variável */
        font-weight: 600;
    }
    ...
    .stAlert {
        border-radius: 12px;
        border-left: 4px solid var(--primary-color); /* Usando variável */
    }
    ...
    hr {
        ...
        border: none;
        height: 2px;
        background: linear-gradient(90deg, transparent, var(--primary-color), transparent); /* Usando variável */
    }
    ...
    ::-webkit-scrollbar-thumb {
        background: var(--secondary-color); /* Usando variável */
        border-radius: 4px;
    }

    ::-webkit-scrollbar-thumb:hover {
        background: var(--primary-color); /* Usando variável */
    }

    /* Estilo para tabelas de métricas */
    .st-emotion-cache-1yr7wds th { /* Cabeçalho da tabela */
        background-color: var(--primary-color) !important;
        color: white !important;
        font-weight: 600 !important;
        text-align: center !important;
    }
    .st-emotion-cache-1yr7wds td { /* Células da tabela */
        text-align: center !important;
        vertical-align: middle !important;
    }
    .st-emotion-cache-1yr7wds tr:nth-child(even) { /* Linhas pares */
        background-color: rgba(107, 155, 127, 0.05) !important;
    }
</style>
...
# NOVO: Input de Múltiplos CNPJs
# Usamos um text_area para permitir que o usuário cole vários CNPJs, um por linha ou separados por vírgula
cnpj_input_raw = st.sidebar.text_area(
    "CNPJs dos Fundos (um por linha ou separados por vírgula)",
    value="",
    placeholder="00.000.000/0000-00\n11.111.111/1111-11",
    help="Digite um ou mais CNPJs. Cada CNPJ deve ter 14 dígitos."
)
...
cnpjs_limpos = []
if cnpj_input_raw:
    # Divide por nova linha ou vírgula, remove espaços e limpa cada CNPJ
    raw_cnpjs = re.split(r'[\n,]', cnpj_input_raw)
    for cnpj in raw_cnpjs:
        cleaned = limpar_cnpj(cnpj.strip())
        if cleaned and len(cleaned) == 14:
            cnpjs_limpos.append(cleaned)
    cnpjs_limpos = list(set(cnpjs_limpos)) # Remove duplicatas

# Validação
cnpj_valido = bool(cnpjs_limpos) # True se houver pelo menos um CNPJ válido
datas_validas = False

if not cnpj_input_raw:
    st.sidebar.error("❌ Por favor, insira pelo menos um CNPJ.")
elif not cnpj_valido:
    st.sidebar.error("❌ Nenhum CNPJ válido encontrado. Verifique se todos têm 14 dígitos.")
else:
    st.sidebar.success(f"✅ CNPJs válidos: {len(cnpjs_limpos)}")

if data_inicial_input and data_final_input:
    if not formatar_data_api(data_inicial_input) or not formatar_data_api(data_final_input):
        st.sidebar.error("❌ Formato de data inválido. Use DD/MM/AAAA")
    else:
        try:
            dt_ini = datetime.strptime(formatar_data_api(data_inicial_input), '%Y%m%d')
            dt_fim = datetime.strptime(formatar_data_api(data_final_input), '%Y%m%d')
            if dt_ini > dt_fim:
...
# Função para carregar dados da API (agora aceita um único CNPJ)
@st.cache_data
def carregar_dados_api_single_fund(cnpj, data_ini_str, data_fim_str):
    ...
    try:
        ...
        # Adiciona o CNPJ e o nome do fundo ao DataFrame para fácil identificação
        df['CNPJ_FUNDO'] = cnpj
        if not df.empty and 'NM_FUNDO' in df.columns:
            df['NM_FUNDO'] = df['NM_FUNDO'].iloc[0] # Garante que o nome seja o mesmo em todas as linhas
        else:
            df['NM_FUNDO'] = f"Fundo {cnpj}" # Fallback se não houver nome

        return df
    except urllib.error.HTTPError as e:
        st.error(f"❌ Erro HTTP ao carregar dados para o CNPJ {cnpj}: {e.code} - {e.reason}. Verifique se o CNPJ está correto ou se há dados disponíveis.")
        return pd.DataFrame()
    except Exception as e:
        st.error(f"❌ Erro ao carregar dados para o CNPJ {cnpj}: {str(e)}")
        return pd.DataFrame()
...
# Gerador de cores para múltiplos fundos
def get_fund_colors(num_funds):
    # Paleta de cores inspirada em tons de verde e azul, com algumas variações
    # Evita cores muito próximas dos benchmarks (preto, azul claro, amarelo)
    base_colors = [
        '#1a5f3f', '#2d8659', '#4a7a5f', '#6b9b7f', '#8ba888', # Tons de verde
        '#345e7e', '#5a809e', '#7a9ebc', # Tons de azul acinzentado
        '#9e7a5a', '#bc9e7a', # Tons terrosos
        '#5a5a5a', '#7a7a7a' # Tons de cinza
    ]
    # Usar itertools.cycle para garantir que sempre haverá cores, mesmo que mais fundos que cores base
    return [color for i, color in zip(range(num_funds), itertools.cycle(base_colors))]
...
if carregar_button and cnpj_valido and datas_validas:
    ...
    st.session_state.cnpjs = cnpjs_limpos
    st.session_state.data_ini = formatar_data_api(data_inicial_input)
    st.session_state.data_fim = formatar_data_api(data_final_input)
...
if not st.session_state.dados_carregados:
    ...
    1.  **CNPJs dos Fundos**: Digite um ou mais CNPJs dos fundos que deseja analisar (um por linha ou separados por vírgula).
...
try:
    with st.spinner('🔄 Carregando dados...'):
        ...
        processed_fund_dfs = []
        fund_names = {} # Para armazenar CNPJ -> Nome do Fundo

        # Cores para os fundos
        fund_colors_list = get_fund_colors(len(st.session_state.cnpjs))
        cnpj_to_color = {cnpj: fund_colors_list[i] for i, cnpj in enumerate(st.session_state.cnpjs)}

        # 1. BAIXAR E PROCESSAR DADOS DE CADA FUNDO
        for i, cnpj in enumerate(st.session_state.cnpjs):
            df_fundo_completo = carregar_dados_api_single_fund(
                cnpj,
            ...
            if df_fundo_completo.empty:
                st.warning(f"⚠️ Nenhum dado encontrado para o CNPJ {cnpj} no período selecionado. Este fundo será ignorado.")
                continue
            ...
            df_processed = df_fundo_completo.copy()

            if not df_cdi_raw.empty:
                df_processed = df_processed.merge(df_cdi_raw[['DT_COMPTC', 'cdi', 'VL_CDI_normalizado']], on='DT_COMPTC', how='left')
            else:
                df_processed.drop(columns=[col for col in ['cdi', 'VL_CDI_normalizado'] if col in df_processed.columns], errors='ignore', inplace=True)

            if not df_ibov_raw.empty:
                df_processed = df_processed.merge(df_ibov_raw[['DT_COMPTC', 'IBOV']], on='DT_COMPTC', how='left')
            else:
                df_processed.drop(columns=[col for col in ['IBOV'] if col in df_processed.columns], errors='ignore', inplace=True)

            df_processed = df_processed.sort_values('DT_COMPTC').reset_index(drop=True)
            ...
            if 'IBOV' in df_processed.columns:
                df_processed['IBOV'] = df_processed['IBOV'].ffill()

            df_processed.dropna(subset=['VL_QUOTA'], inplace=True)

            df_processed = df_processed[(df_processed['DT_COMPTC'] >= dt_ini_user) & (df_processed['DT_COMPTC'] <= dt_fim_user)].copy()

            if df_processed.empty:
                st.warning(f"⚠️ Nenhum dado disponível para o CNPJ {cnpj} no período selecionado após a combinação com os indicadores. Este fundo será ignorado.")
                continue

            # 7. Re-normalizar a cota do fundo para começar em 1.0 (0% de rentabilidade) na primeira data do 'df' final
            primeira_cota_fundo = df_processed['VL_QUOTA'].iloc[0]
            df_processed['VL_QUOTA_NORM'] = ((df_processed['VL_QUOTA'] / primeira_cota_fundo) - 1) * 100

            tem_cdi = False
            if st.session_state.mostrar_cdi and 'VL_CDI_normalizado' in df_processed.columns and not df_processed['VL_CDI_normalizado'].isna().all():
                first_cdi_normalized_value_in_period = df_processed['VL_CDI_normalizado'].iloc[0]
                df_processed['CDI_COTA'] = df_processed['VL_CDI_normalizado'] / first_cdi_normalized_value_in_period
                df_processed['CDI_NORM'] = (df_processed['CDI_COTA'] - 1) * 100
                tem_cdi = True
            else:
                df_processed.drop(columns=[col for col in ['cdi', 'VL_CDI_normalizado', 'CDI_COTA', 'CDI_NORM'] if col in df_processed.columns], errors='ignore', inplace=True)

            tem_ibov = False
            if st.session_state.mostrar_ibov and 'IBOV' in df_processed.columns and not df_processed['IBOV'].isna().all():
                first_ibov_value = df_processed['IBOV'].iloc[0]
                if first_ibov_value and not pd.isna(first_ibov_value):
                    df_processed['IBOV_COTA'] = df_processed['IBOV'] / first_ibov_value
                    df_processed['IBOV_NORM'] = (df_processed['IBOV_COTA'] - 1) * 100
                    tem_ibov = True
            else:
                df_processed.drop(columns=[col for col in ['IBOV', 'IBOV_COTA', 'IBOV_NORM'] if col in df_processed.columns], errors='ignore', inplace=True)

            # 3. CALCULAR MÉTRICAS (agora usando o 'df_processed' combinado e normalizado)
            df_processed = df_processed.sort_values('DT_COMPTC').reset_index(drop=True)

            # Métricas do fundo
            df_processed['Max_VL_QUOTA'] = df_processed['VL_QUOTA'].cummax()
            df_processed['Drawdown'] = (df_processed['VL_QUOTA'] / df_processed['Max_VL_QUOTA'] - 1) * 100
            df_processed['Captacao_Liquida'] = df_processed['CAPTC_DIA'] - df_processed['RESG_DIA']
            df_processed['Soma_Acumulada'] = df_processed['Captacao_Liquida'].cumsum()
            df_processed['Patrimonio_Liq_Medio'] = df_processed['VL_PATRIM_LIQ'] / df_processed['NR_COTST']

            vol_window = 21
            trading_days_in_year = 252 # Número de dias úteis em um ano para anualização
            df_processed['Variacao_Perc'] = df_processed['VL_QUOTA'].pct_change()
            df_processed['Volatilidade'] = df_processed['Variacao_Perc'].rolling(vol_window).std() * np.sqrt(trading_days_in_year) * 100

            # Volatilidade histórica para o card
            vol_hist = round(df_processed['Variacao_Perc'].std() * np.sqrt(trading_days_in_year) * 100, 2)
            df_processed['Vol_Hist_Fundo'] = vol_hist # Armazena para o card

            # CAGR - Cálculo conforme sua especificação: última cota fixa, cota inicial variável
            df_processed['CAGR_Fundo'] = np.nan
            if tem_cdi:
                df_processed['CAGR_CDI'] = np.nan
            if tem_ibov:
                df_processed['CAGR_IBOV'] = np.nan

            if not df_processed.empty and len(df_processed) > trading_days_in_year:
                end_value_fundo = df_processed['VL_QUOTA'].iloc[-1]
                ...
                # O range vai de 0 até (len(df_processed) - trading_days_in_year)
                for i in range(len(df_processed) - trading_days_in_year):
                    initial_value_fundo = df_processed['VL_QUOTA'].iloc[i]
                    num_intervals = (len(df_processed) - 1) - i # Número de dias entre o ponto atual e o final

                    if initial_value_fundo > 0 and num_intervals >= trading_days_in_year:
                        df_processed.loc[i, 'CAGR_Fundo'] = ((end_value_fundo / initial_value_fundo) ** (trading_days_in_year / num_intervals) - 1) * 100

                    if tem_cdi and 'CDI_COTA' in df_processed.columns:
                        initial_value_cdi = df_processed['CDI_COTA'].iloc[i]
                        end_value_cdi = df_processed['CDI_COTA'].iloc[-1]
                        if initial_value_cdi > 0 and num_intervals >= trading_days_in_year:
                            df_processed.loc[i, 'CAGR_CDI'] = ((end_value_cdi / initial_value_cdi) ** (trading_days_in_year / num_intervals) - 1) * 100

                    if tem_ibov and 'IBOV_COTA' in df_processed.columns:
                        initial_value_ibov = df_processed['IBOV_COTA'].iloc[i]
                        end_value_ibov = df_processed['IBOV_COTA'].iloc[-1]
                        if initial_value_ibov > 0 and num_intervals >= trading_days_in_year:
                            df_processed.loc[i, 'CAGR_IBOV'] = ((end_value_ibov / initial_value_ibov) ** (trading_days_in_year / num_intervals) - 1) * 100

            # Calcular CAGR médio para o card de métricas (baseado na nova coluna CAGR_Fundo)
            mean_cagr = df_processed['CAGR_Fundo'].mean() if 'CAGR_Fundo' in df_processed.columns else 0
            if pd.isna(mean_cagr):
                mean_cagr = 0
            df_processed['Mean_CAGR_Fundo'] = mean_cagr # Armazena para o card

            # VaR
            df_processed['Retorno_21d'] = df_processed['VL_QUOTA'].pct_change(21)
            df_plot_var = df_processed.dropna(subset=['Retorno_21d']).copy()
            ...
            df_processed['VaR_95'] = VaR_95
            df_processed['VaR_99'] = VaR_99
            df_processed['ES_95'] = ES_95
            df_processed['ES_99'] = ES_99

            # Armazena o DataFrame processado e o nome do fundo
            processed_fund_dfs.append(df_processed)
            fund_names[cnpj] = df_processed['NM_FUNDO'].iloc[0] if 'NM_FUNDO' in df_processed.columns else f"Fundo {cnpj}"

        if not processed_fund_dfs:
            st.error("❌ Nenhum fundo pôde ser carregado ou processado com sucesso. Verifique os CNPJs e o período.")
            st.stop()

    num_fundos_selecionados = len(processed_fund_dfs)

    # Cores (mantidas as mesmas para benchmarks)
    color_primary = '#1a5f3f'  # Verde escuro para o fundo (se for um só)
    color_secondary = '#6b9b7f' # Verde claro para o patrimônio (se for um só)
    color_danger = '#dc3545' # Vermelho para drawdown
    color_cdi = '#000000'  # Preto para o CDI
    color_ibov = '#007bff' # Azul para o Ibovespa (alterado de amarelo para azul para melhor contraste e padrão)

    # Cards de métricas (apenas para um único fundo)
    if num_fundos_selecionados == 1:
        df = processed_fund_dfs[0] # Pega o único DataFrame
        st.subheader(f"Métricas Chave para {fund_names[st.session_state.cnpjs[0]]}")
        ...
        with col3:
            st.metric("CAGR Médio", fmt_pct_port(df['Mean_CAGR_Fundo'] / 100))
        with col4:
            st.metric("Max Drawdown", fmt_pct_port(df['Drawdown'].min() / 100))
        with col5:
            st.metric("Vol. Histórica", fmt_pct_port(df['Vol_Hist_Fundo']/100))
    else:
        st.info("ℹ️ Os cards de métricas rápidas são exibidos apenas quando um único fundo é selecionado.")
    ...
    with tab1:
        ...
        for i, df_fundo in enumerate(processed_fund_dfs):
            cnpj = df_fundo['CNPJ_FUNDO'].iloc[0]
            fund_name = fund_names[cnpj]
            current_fund_color = cnpj_to_color[cnpj]

            fig1.add_trace(go.Scatter(
                x=df_fundo['DT_COMPTC'],
                y=df_fundo['VL_QUOTA_NORM'],
                mode='lines',
                name=f'{fund_name}',
                line=dict(color=current_fund_color, width=2.5),
                fill='tozeroy',
                fillcolor=f'rgba({int(current_fund_color[1:3], 16)}, {int(current_fund_color[3:5], 16)}, {int(current_fund_color[5:7], 16)}, 0.1)',
                hovertemplate=f'<b>{fund_name}</b><br>Data: %{{x|%d/%m/%Y}}<br>Rentabilidade: %{{y:.2f}}%<extra></extra>'
            ))

        if st.session_state.mostrar_cdi and 'CDI_NORM' in processed_fund_dfs[0].columns:
            # Pega o CDI de um dos fundos (eles devem ser os mesmos)
            df_benchmark_ref = processed_fund_dfs[0]
            fig1.add_trace(go.Scatter(
                x=df_benchmark_ref['DT_COMPTC'],
                y=df_benchmark_ref['CDI_NORM'],
                mode='lines',
                name='CDI',
        ...
        if st.session_state.mostrar_ibov and 'IBOV_NORM' in processed_fund_dfs[0].columns:
            df_benchmark_ref = processed_fund_dfs[0]
            fig1.add_trace(go.Scatter(
                x=df_benchmark_ref['DT_COMPTC'],
                y=df_benchmark_ref['IBOV_NORM'],
                mode='lines',
                name='Ibovespa',
        ...
        # Ajusta o range do eixo X para os dados de df (usando o primeiro fundo como referência de data)
        fig1 = add_watermark_and_style(fig1, logo_base64, x_range=[processed_fund_dfs[0]['DT_COMPTC'].min(), processed_fund_dfs[0]['DT_COMPTC'].max()], x_autorange=False)
        ...
        for i, df_fundo in enumerate(processed_fund_dfs):
            ...
            df_plot_cagr_fundo = df_fundo.dropna(subset=['CAGR_Fundo']).copy()
            if not df_plot_cagr_fundo.empty:
                fig2.add_trace(go.Scatter(
                    x=df_plot_cagr_fundo['DT_COMPTC'],
                    y=df_plot_cagr_fundo['CAGR_Fundo'],
                    mode='lines',
                    name=f'CAGR {fund_name}',
                    line=dict(color=current_fund_color, width=2.5),
                    hovertemplate=f'<b>CAGR {fund_name}</b><br>Data: %{{x|%d/%m/%Y}}<br>CAGR: %{{y:.2f}}%<extra></extra>'
                ))
                # Adiciona a linha do CAGR médio para cada fundo
                mean_cagr_fundo = df_fundo['Mean_CAGR_Fundo'].iloc[0]
                fig2.add_trace(go.Scatter(
                    x=df_plot_cagr_fundo['DT_COMPTC'],
                    y=[mean_cagr_fundo] * len(df_plot_cagr_fundo),
                    mode='lines',
                    line=dict(dash='dash', color=current_fund_color, width=1.5),
                    name=f'CAGR Médio {fund_name} ({mean_cagr_fundo:.2f}%)'
                ))
            else:
                st.warning(f"⚠️ Não há dados suficientes para calcular o CAGR para {fund_name} (mínimo de 1 ano de dados).")

        # Benchmarks (CDI e Ibovespa)
        df_benchmark_ref = processed_fund_dfs[0] # Usa o primeiro fundo como referência para datas
        if st.session_state.mostrar_cdi and 'CAGR_CDI' in df_benchmark_ref.columns:
            df_plot_cagr_cdi = df_benchmark_ref.dropna(subset=['CAGR_CDI']).copy()
        ...
        if st.session_state.mostrar_ibov and 'CAGR_IBOV' in df_benchmark_ref.columns:
            df_plot_cagr_ibov = df_benchmark_ref.dropna(subset=['CAGR_IBOV']).copy()
            if not df_plot_cagr_ibov.empty:
                fig2.add_trace(go.Scatter(
                    x=df_plot_cagr_ibov['DT_COMPTC'],
                    y=df_plot_cagr_ibov['CAGR_IBOV'],
                    mode='lines',
        ...
        # Ajusta o range do eixo X para os dados de df_plot_cagr (usando o primeiro fundo como referência)
        if not processed_fund_dfs[0].dropna(subset=['CAGR_Fundo']).empty:
            fig2 = add_watermark_and_style(fig2, logo_base64, x_range=[processed_fund_dfs[0]['DT_COMPTC'].min(), processed_fund_dfs[0]['DT_COMPTC'].max()], x_autorange=False)
        else:
        ...
        if num_fundos_selecionados > 1:
            st.info("ℹ️ Para visualizar o Excesso de Retorno Anualizado, selecione apenas um fundo.")
        elif (st.session_state.mostrar_cdi and not st.session_state.mostrar_ibov) or (st.session_state.mostrar_ibov and not st.session_state.mostrar_cdi):
            df = processed_fund_dfs[0]
            benchmark_cagr_col = ''
            benchmark_name = ''
            if st.session_state.mostrar_cdi:
                benchmark_cagr_col = 'CAGR_CDI'
                benchmark_name = 'CDI'
            elif st.session_state.mostrar_ibov:
        ...
        elif st.session_state.mostrar_cdi and st.session_state.mostrar_ibov:
    ...
    with tab2:
        ...
        for i, df_fundo in enumerate(processed_fund_dfs):
            ...
            fig3.add_trace(go.Scatter(
                ...
                y=df_fundo['Drawdown'],
                mode='lines',
                name=f'Drawdown {fund_name}',
                line=dict(color=current_fund_color, width=2.5),
                # fill='tozeroy', # Remover fill para múltiplos fundos
                # fillcolor='rgba(220, 53, 69, 0.1)',
                hovertemplate=f'<b>Drawdown {fund_name}</b><br>Data: %{{x|%d/%m/%Y}}<br>Drawdown: %{{y:.2f}}%<extra></extra>'
            ))
        ...
        fig3 = add_watermark_and_style(fig3, logo_base64, x_range=[processed_fund_dfs[0]['DT_COMPTC'].min(), processed_fund_dfs[0]['DT_COMPTC'].max()], x_autorange=False)
        ...
        for i, df_fundo in enumerate(processed_fund_dfs):
            ...
            fund_name = fund_names[cnpj]
            current_fund_color = cnpj_to_color[cnpj]
            vol_hist_fundo = df_fundo['Vol_Hist_Fundo'].iloc[0]

            fig4.add_trace(go.Scatter(
                x=df_fundo['DT_COMPTC'],
                y=df_fundo['Volatilidade'],
                mode='lines',
                name=f'Volatilidade {fund_name} ({vol_window} dias)',
                line=dict(color=current_fund_color, width=2.5),
                # fill='tozeroy', # Remover fill para múltiplos fundos
                # fillcolor='rgba(26, 95, 63, 0.1)',
                hovertemplate=f'<b>Volatilidade {fund_name}</b><br>Data: %{{x|%d/%m/%Y}}<br>Volatilidade: %{{y:.2f}}%<extra></extra>'
            ))
            fig4.add_trace(go.Scatter(
                x=df_fundo['DT_COMPTC'],
                y=[vol_hist_fundo] * len(df_fundo),
                mode='lines',
                line=dict(dash='dash', color=current_fund_color, width=1.5),
                name=f'Vol. Histórica {fund_name} ({vol_hist_fundo:.2f}%)'
            ))
        ...
        fig4 = add_watermark_and_style(fig4, logo_base64, x_range=[processed_fund_dfs[0]['DT_COMPTC'].min(), processed_fund_dfs[0]['DT_COMPTC'].max()], x_autorange=False)
        ...
        if num_fundos_selecionados > 1:
            ...
            for i, df_fundo in enumerate(processed_fund_dfs):
                ...
                VaR_95 = df_fundo['VaR_95'].iloc[0]
                VaR_99 = df_fundo['VaR_99'].iloc[0]
                ES_95 = df_fundo['ES_95'].iloc[0]
                ES_99 = df_fundo['ES_99'].iloc[0]

                var_es_data.append({
                    'Métrica': 'VaR 95%',
                    fund_name: f"{VaR_95 * 100:.2f}%" if not pd.isna(VaR_95) else "N/A"
                })
                var_es_data.append({
                    'Métrica': 'ES 95%',
                    fund_name: f"{ES_95 * 100:.2f}%" if not pd.isna(ES_95) else "N/A"
                })
                var_es_data.append({
                    'Métrica': 'VaR 99%',
                    fund_name: f"{VaR_99 * 100:.2f}%" if not pd.isna(VaR_99) else "N/A"
                })
                var_es_data.append({
                    'Métrica': 'ES 99%',
                    fund_name: f"{ES_99 * 100:.2f}%" if not pd.isna(ES_99) else "N/A"
                })

            df_var_es_table = pd.DataFrame(var_es_data)
            # Agrupa e transpoe para ter Métricas nas linhas e Fundos nas colunas
            df_var_es_table_pivot = df_var_es_table.set_index('Métrica').T
            st.table(df_var_es_table_pivot)
        else: # Lógica para um único fundo (manter o gráfico e texto explicativo)
            df = processed_fund_dfs[0]
            df_plot_var = df.dropna(subset=['Retorno_21d']).copy()
            VaR_95 = df['VaR_95'].iloc[0]
            VaR_99 = df['VaR_99'].iloc[0]
            ES_95 = df['ES_95'].iloc[0]
            ES_99 = df['ES_99'].iloc[0]
        ...
        if num_fundos_selecionados > 1:
            if (st.session_state.mostrar_cdi and st.session_state.mostrar_ibov):
            ...
            elif not st.session_state.mostrar_cdi and not st.session_state.mostrar_ibov:
                ...
                for i, df_fundo in enumerate(processed_fund_dfs):
                    ...
                    if not df_fundo.empty and len(df_fundo) > trading_days_in_year and benchmark_cota_col in df_fundo.columns:
                        total_fund_return = (df_fundo['VL_QUOTA'].iloc[-1] / df_fundo['VL_QUOTA'].iloc[0]) - 1
                        total_benchmark_return = (df_fundo[benchmark_cota_col].iloc[-1] / df_fundo[benchmark_cota_col].iloc[0]) - 1

                        num_days_in_period = len(df_fundo)
                        ...
                        annualized_fund_volatility = df_fundo['Vol_Hist_Fundo'].iloc[0] / 100 if not pd.isna(df_fundo['Vol_Hist_Fundo'].iloc[0]) else np.nan
                        max_drawdown_value = df_fundo['Drawdown'].min() / 100 if not df_fundo['Drawdown'].empty else np.nan
                        cagr_fund_decimal = df_fundo['Mean_CAGR_Fundo'].iloc[0] / 100 if not pd.isna(df_fundo['Mean_CAGR_Fundo'].iloc[0]) else np.nan

                        drawdown_series = (df_fundo['VL_QUOTA'] / df_fundo['Max_VL_QUOTA'] - 1)
                        ...
                        if benchmark_daily_rate_col and benchmark_daily_rate_col in df_fundo.columns:
                            excess_returns_vs_benchmark_daily = df_fundo['Variacao_Perc'] - (df_fundo[benchmark_daily_rate_col] / 100)
                            downside_returns = excess_returns_vs_benchmark_daily[excess_returns_vs_benchmark_daily < 0]
                        else:
                            downside_returns = df_fundo['Variacao_Perc'][df_fundo['Variacao_Perc'] < 0]
                        ...
                        if benchmark_daily_rate_col and benchmark_daily_rate_col in df_fundo.columns and not df_fundo['Variacao_Perc'].empty:
                            excess_daily_returns = df_fundo['Variacao_Perc'] - (df_fundo[benchmark_daily_rate_col] / 100)
                            if not excess_daily_returns.empty:
                                tracking_error = excess_daily_returns.std() * np.sqrt(trading_days_in_year)
                            else:
                                tracking_error = np.nan
                        elif benchmark_cota_col in df_fundo.columns and not df_fundo['Variacao_Perc'].empty:
                            benchmark_daily_returns = df_fundo[benchmark_cota_col].pct_change()
                            excess_daily_returns = df_fundo['Variacao_Perc'] - benchmark_daily_returns
                            if not excess_daily_returns.empty:
                                tracking_error = excess_daily_returns.std() * np.sqrt(trading_days_in_year)
                            else:
                                tracking_error = np.nan
                    ...
                    metrics_data_list.append({
                        ...
                        fund_name: f"{sharpe_ratio:.2f}" if not pd.isna(sharpe_ratio) else "N/A"
                    })
                    metrics_data_list.append({
                        'Métrica': 'Sortino Ratio',
                        fund_name: f"{sortino_ratio:.2f}" if not pd.isna(sortino_ratio) else "N/A"
                    })
                    metrics_data_list.append({
                        'Métrica': 'Information Ratio',
                        fund_name: f"{information_ratio:.2f}" if not pd.isna(information_ratio) else "N/A"
                    })
                    metrics_data_list.append({
                        'Métrica': 'Calmar Ratio',
                        fund_name: f"{calmar_ratio:.2f}" if not pd.isna(calmar_ratio) else "N/A"
                    })
                    metrics_data_list.append({
                        'Métrica': 'Sterling Ratio',
                        fund_name: f"{sterling_ratio:.2f}" if not pd.isna(sterling_ratio) else "N/A"
                    })
                    metrics_data_list.append({
                        'Métrica': 'Ulcer Index',
                        fund_name: f"{ulcer_index:.2f}" if not pd.isna(ulcer_index) else "N/A"
                    })
                    metrics_data_list.append({
                        'Métrica': 'Martin Ratio',
                        fund_name: f"{martin_ratio:.2f}" if not pd.isna(martin_ratio) else "N/A"
                    })

                df_metrics_table = pd.DataFrame(metrics_data_list)
                # Agrupa e transpoe para ter Métricas nas linhas e Fundos nas colunas
                df_metrics_table_pivot = df_metrics_table.set_index('Métrica').T
                st.table(df_metrics_table_pivot)

        else: # Lógica para um único fundo (manter os cards e explicações)
            df = processed_fund_dfs[0]
            if (st.session_state.mostrar_cdi and st.session_state.mostrar_ibov):
                ...
                if not df.empty and len(df) > trading_days_in_year and benchmark_cota_col in df.columns:
                    ...
                    annualized_fund_volatility = df['Vol_Hist_Fundo'].iloc[0] / 100 if not pd.isna(df['Vol_Hist_Fundo'].iloc[0]) else np.nan
                    max_drawdown_value = df['Drawdown'].min() / 100 if not df['Drawdown'].empty else np.nan
                    cagr_fund_decimal = df['Mean_CAGR_Fundo'].iloc[0] / 100 if not pd.isna(df['Mean_CAGR_Fundo'].iloc[0]) else np.nan
                    ...
                    if benchmark_daily_rate_col and benchmark_daily_rate_col in df.columns and not df['Variacao_Perc'].empty:
                        ...
                        if not excess_daily_returns.empty:
                            ...
                            tracking_error = np.nan
                    elif benchmark_cota_col in df.columns and not df['Variacao_Perc'].empty:
                    ...
                    with col_vol_3:
                        ...
                        *   **Interpretação Geral:**
                        ...
                        """)
                    with col_vol_4:
                        st.metric("Treynor Ratio", "Não Calculável" if benchmark_name != 'Ibovespa' else "N/A")
                        st.info("""
    ...
    with tab3:
        st.subheader("Patrimônio Líquido dos Fundos")
        ...
        for i, df_fundo in enumerate(processed_fund_dfs):
            ...
            fig6.add_trace(go.Scatter(
                x=df_fundo['DT_COMPTC'],
                y=df_fundo['VL_PATRIM_LIQ'],
                mode='lines',
                name=f'Patrimônio Líquido {fund_name}',
                line=dict(color=current_fund_color, width=2.5),
                hovertemplate=f'Data: %{{x|%d/%m/%Y}}<br>Patrimônio Líquido {fund_name}: %{{customdata}}<extra></extra>',
                customdata=[format_brl(v) for v in df_fundo['VL_PATRIM_LIQ']]
            ))
        ...
        fig6 = add_watermark_and_style(fig6, logo_base64, x_range=[processed_fund_dfs[0]['DT_COMPTC'].min(), processed_fund_dfs[0]['DT_COMPTC'].max()], x_autorange=False)
        st.plotly_chart(fig6, use_container_width=True)

        st.subheader("Captação Líquida Mensal")

        if num_fundos_selecionados > 1:
            st.info("ℹ️ Para visualizar a Captação Líquida Mensal, selecione apenas um fundo.")
        else:
            df = processed_fund_dfs[0]
    ...
    with tab4:
        st.subheader("Número de Cotistas dos Fundos")
        ...
        for i, df_fundo in enumerate(processed_fund_dfs):
            ...
            fig8.add_trace(go.Scatter(
                ...
                y=df_fundo['NR_COTST'],
                mode='lines',
                name=f'Número de Cotistas {fund_name}',
                line=dict(color=current_fund_color, width=2.5),
                hovertemplate=f'Data: %{{x|%d/%m/%Y}}<br>Nº de Cotistas {fund_name}: %{{y}}<extra></extra>'
            ))

        if num_fundos_selecionados == 1: # Se for um único fundo, adiciona o Patrimônio Médio
            df = processed_fund_dfs[0]
            fig8.add_trace(go.Scatter(
                x=df['DT_COMPTC'],
            ...
            fig8.update_layout(
                yaxis=dict(title="Número de Cotistas"),
                yaxis2=dict(title="Patrimônio Médio por Cotista (R$)", overlaying="y", side="right"),
            )
        else:
            fig8.update_layout(
                yaxis=dict(title="Número de Cotistas"),
        ...
        fig8 = add_watermark_and_style(fig8, logo_base64, x_range=[processed_fund_dfs[0]['DT_COMPTC'].min(), processed_fund_dfs[0]['DT_COMPTC'].max()], x_autorange=False)
    ...
    with tab5:
        ...
        # Calcular retornos em janelas móveis para cada fundo
        for df_fundo in processed_fund_dfs:
            for nome, dias in janelas.items():
                if len(df_fundo) > dias:
                    df_fundo[f'FUNDO_{nome}'] = df_fundo['VL_QUOTA'] / df_fundo['VL_QUOTA'].shift(dias) - 1
                    if st.session_state.mostrar_cdi and 'CDI_COTA' in df_fundo.columns:
                        df_fundo[f'CDI_{nome}'] = df_fundo['CDI_COTA'] / df_fundo['CDI_COTA'].shift(dias) - 1
                    if st.session_state.mostrar_ibov and 'IBOV_COTA' in df_fundo.columns:
                        df_fundo[f'IBOV_{nome}'] = df_fundo['IBOV_COTA'] / df_fundo['IBOV_COTA'].shift(dias) - 1
                else:
                    df_fundo[f'FUNDO_{nome}'] = np.nan
                    if st.session_state.mostrar_cdi:
                        df_fundo[f'CDI_{nome}'] = np.nan
                    if st.session_state.mostrar_ibov:
                        df_fundo[f'IBOV_{nome}'] = np.nan

        if num_fundos_selecionados > 1:
            st.info("ℹ️ Para visualizar os Retornos em Janelas Móveis, selecione apenas um fundo.")
        else:
            df = processed_fund_dfs[0]
            janela_selecionada = st.selectbox("Selecione o período:", list(janelas.keys()))

            if not df[f'FUNDO_{janela_selecionada}'].dropna().empty:
                fig9 = go.Figure()

                fig9.add_trace(go.Scatter(
                    x=df['DT_COMPTC'],
                    y=df[f'FUNDO_{janela_selecionada}'],
                    mode='lines',
                ...
                if st.session_state.mostrar_cdi:
                    fig9.add_trace(go.Scatter(
                        x=df['DT_COMPTC'],
                        y=df[f'CDI_{janela_selecionada}'],
                        mode='lines',
                ...
                if st.session_state.mostrar_ibov:
                    fig9.add_trace(go.Scatter(
                        x=df['DT_COMPTC'],
                        y=df[f'IBOV_{janela_selecionada}'],
                        mode='lines',
                ...
                df_plot_returns = df.dropna(subset=[f'FUNDO_{janela_selecionada}']).copy()
        ...
        if num_fundos_selecionados > 1:
            if (st.session_state.mostrar_cdi and st.session_state.mostrar_ibov):
                ...
                for nome_janela, dias in janelas.items():
                    row_data = {'Janela': nome_janela.split(' ')[0]}

                    for df_fundo_atual in processed_fund_dfs:
                        cnpj_fundo_atual = df_fundo_atual['CNPJ_FUNDO'].iloc[0]
                        fund_name_display = fund_names[cnpj_fundo_atual]

                        fund_returns_col = f'FUNDO_{nome_janela}'
                        benchmark_col = f'{benchmark_prefix}_{nome_janela}'

                        if fund_returns_col in df_fundo_atual.columns and benchmark_col in df_fundo_atual.columns:
                            temp_df = df_fundo_atual[[fund_returns_col, benchmark_col]].dropna()
                            if not temp_df.empty:
                                outperformed_count = (temp_df[fund_returns_col] > temp_df[benchmark_col]).sum()
                                ...
                                row_data[f'{fund_name_display} vs {benchmark_name_consistency}'] = f"{consistency_percentage:.2f}%" if not pd.isna(consistency_percentage) else "N/A"
                            else:
                                row_data[f'{fund_name_display} vs {benchmark_name_consistency}'] = "N/A"
                        else:
                            row_data[f'{fund_name_display} vs {benchmark_name_consistency}'] = "N/A"

                    consistency_table_data.append(row_data)

                df_consistency_table = pd.DataFrame(consistency_table_data)
                st.table(df_consistency_table)

        else: # Lógica para um único fundo (manter o gráfico de barras)
            df = processed_fund_dfs[0]
            if (st.session_state.mostrar_cdi and st.session_state.mostrar_ibov):
                st.info("ℹ️ Para visualizar a Consistência em Janelas Móveis, selecione apenas um indicador de comparação (CDI ou Ibovespa) na barra lateral.")
            elif not st.session_state.mostrar_cdi and not st.session_state.mostrar_ibov:
                ...
                for nome, dias in janelas.items():
                    ...
                    if fund_col in df.columns and benchmark_col in df.columns:
                        temp_df = df[[fund_col, benchmark_col]].dropna()
    ...
    st.error(f"❌ Erro geral ao processar os dados: {str(e)}")
