...
def carregar_dados_api(cnpj, data_ini_str, data_fim_str):
    ...
    # ATENÇÃO: Verifique se este token de autorização está correto e ativo.
    # Tokens Bearer geralmente são sequências de caracteres mais complexas.
...
def calculate_single_fund_metrics(df_fund, df_cdi_period, fund_name, dt_ini_user, dt_fim_user, tem_cdi):
    ...
    # Métricas básicas
    metrics["Patrimônio Líquido"] = df_fund_filtered['VL_PATRIM_LIQ'].iloc[-1] if not df_fund_filtered['VL_PATRIM_LIQ'].empty else np.nan
    metrics["Rentabilidade Acumulada"] = df_fund_filtered['VL_QUOTA_NORM'].iloc[-1] / 100 if not df_fund_filtered['VL_QUOTA_NORM'].empty else np.nan
    ...
    metrics["Max Drawdown"] = df_fund_filtered['Drawdown'].min() / 100 if not df_fund_filtered['Drawdown'].empty else np.nan

    df_fund_filtered['Variacao_Perc'] = df_fund_filtered['VL_QUOTA'].pct_change()
    vol_hist = df_fund_filtered['Variacao_Perc'].std() * np.sqrt(trading_days_in_year) * 100 if not df_fund_filtered['Variacao_Perc'].empty else np.nan
    metrics["Vol. Histórica"] = vol_hist / 100 if not pd.isna(vol_hist) else np.nan
    ...
    mean_cagr = df_fund_filtered['CAGR_Fundo'].mean() if 'CAGR_Fundo' in df_fund_filtered.columns and not df_fund_filtered['CAGR_Fundo'].empty else np.nan
    metrics["CAGR Médio"] = mean_cagr / 100 if not pd.isna(mean_cagr) else np.nan
    ...
    if tem_cdi and not df_cdi_period.empty:
        ...
        first_cdi_normalized_value_in_period = df_merged['VL_CDI_normalizado'].iloc[0] if not df_merged['VL_CDI_normalizado'].empty else np.nan
        if not pd.isna(first_cdi_normalized_value_in_period) and first_cdi_normalized_value_in_period != 0:
        ...
        if len(df_merged) > trading_days_in_year and 'CDI_COTA' in df_merged.columns:
        ...
        mean_cagr_cdi = df_merged['CAGR_CDI'].mean() if 'CAGR_CDI' in df_merged.columns and not df_merged['CAGR_CDI'].empty else np.nan

        # Retorno total do fundo e CDI no período
        total_fund_return = (df_merged['VL_QUOTA'].iloc[-1] / df_merged['VL_QUOTA'].iloc[0]) - 1 if not df_merged['VL_QUOTA'].empty and df_merged['VL_QUOTA'].iloc[0] != 0 else np.nan
        total_cdi_return = (df_merged['CDI_COTA'].iloc[-1] / df_merged['CDI_COTA'].iloc[0]) - 1 if 'CDI_COTA' in df_merged.columns and not df_merged['CDI_COTA'].empty and df_merged['CDI_COTA'].iloc[0] != 0 else np.nan

        # Anualização dos retornos totais para consistência
        num_days_in_period = len(df_merged)
        if num_days_in_period > 0 and not pd.isna(total_fund_return):
            annualized_fund_return = (1 + total_fund_return)**(trading_days_in_year / num_days_in_period) - 1
        else:
            annualized_fund_return = np.nan

        if num_days_in_period > 0 and not pd.isna(total_cdi_return):
        ...
        # --- Cálculo dos Ratios ---
        sharpe_ratio = (annualized_fund_return - annualized_cdi_return) / annualized_fund_volatility if not pd.isna(annualized_fund_return) and not pd.isna(annualized_cdi_return) and not pd.isna(annualized_fund_volatility) and annualized_fund_volatility != 0 else np.nan
        sortino_ratio = (annualized_fund_return - annualized_cdi_return) / annualized_downside_volatility if not pd.isna(annualized_fund_return) and not pd.isna(annualized_cdi_return) and not pd.isna(annualized_downside_volatility) and annualized_downside_volatility != 0 else np.nan
        information_ratio = (annualized_fund_return - annualized_cdi_return) / tracking_error if not pd.isna(annualized_fund_return) and not pd.isna(annualized_cdi_return) and not pd.isna(tracking_error) and tracking_error != 0 else np.nan
    ...
    for i, cnpj in enumerate(st.session_state.cnpjs_carregados):
        ...
        if tem_cdi:
            ...
            if not df_fundo_filtered['VL_CDI_normalizado'].dropna().empty:
                ...
                if first_cdi_normalized_value_in_period != 0:
