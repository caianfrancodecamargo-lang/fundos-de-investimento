...
# --- Lógica de Carregamento de Dados (acionada pelo botão) ---
if carregar_button:
    cnpj_limpo = limpar_cnpj(cnpj_input)
    data_inicial_api = formatar_data_api(data_inicial_input)
    data_final_api = formatar_data_api(data_final_input)

    st.session_state['dados_carregados'] = False # Reseta o estado de carregamento
    ...
    # Se a validação passou, tenta carregar os dados
    with st.spinner("Carregando dados do fundo e CDI..."):
        try:
            ...
            start_year_fetch = data_inicio_fundo_ampliado.year
            end_year_fetch = dt_fim_user.year

            for year in range(start_year_fetch, end_year_fetch + 1):
                start_month = data_inicio_fundo_ampliado.month if year == start_year_fetch else 1
                end_month = dt_fim_user.month if year == end_year_fetch else 12
            ...
            if df_fundo_completo.empty:
                raise ValueError(f"CNPJ {cnpj_input} não encontrado ou sem dados para o período ampliado.")
            ...
            if st.session_state['tem_cdi']:
                ...
                if df_final.empty:
                    raise ValueError("Não há dados do fundo disponíveis para o período selecionado, mesmo após preenchimento com CDI.")
                ...
                if df_final.empty:
                    raise ValueError("Não há dados do fundo disponíveis para o período selecionado após o filtro final.")
                ...
                if df_final.empty:
                    raise ValueError("Não há dados do fundo disponíveis para o período selecionado.")
            ...
            st.session_state['dados_carregados'] = True # Marca como carregado com sucesso
...
# --- Exibição do Dashboard (condicional) ---
...
# Agora, todo o restante do dashboard será envolvido em um try-except para capturar erros de exibição/cálculo.
try:
    ...
    st.error(f"❌ Erro ao exibir o dashboard: {str(e)}")
    st.info("💡 Um erro inesperado ocorreu durante a exibição. Tente recarregar os dados ou ajustar o período.")
    st.session_state['dados_carregados'] = False # Reset para voltar ao estado inicial

# Footer (sempre exibido)
st.markdown("---")
st.markdown("""
